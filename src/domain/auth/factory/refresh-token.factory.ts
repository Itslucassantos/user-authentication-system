...
export default class RefreshTokenFactory {
    static create(userId: string, clientApplicationId: string, tokenHash: string, deviceInfo: string): RefreshToken {
        const expiresAt = new Date(Date.now() + REFRESH_TOKEN_TTL_MS);
        return new RefreshToken(uuid(), userId, clientApplicationId, tokenHash, deviceInfo, expiresAt);
    }

    static restore(id: string, userId: string, clientApplicationId: string, tokenHash: string, deviceInfo: string, expiresAt: Date): RefreshToken {
